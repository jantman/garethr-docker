...
define docker::run(
  ...
  $volumes_from = [],
  $net = 'bridge',
  Optional[Variant[String, Boolean]] $username = false,
  Optional[Variant[String, Boolean]] $hostname = false,
  $env = [],
  $env_file = [],
  $dns = [],
  $dns_search = [],
  ...
  Optional[Pattern[/^(no|always|on-success|on-failure|on-abnormal|on-abort|on-watchdog)$/]] $systemd_restart = 'on-failure',
  ...
  $socket_connect = [],
  $hostentries = [],
  Optional[Pattern[/^(no|always|unless-stopped|on-failure)|^on-failure:[\d]+$/]] $restart = undef,
  $before_start = false,
  $before_stop = false,
